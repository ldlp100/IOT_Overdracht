...
{
    public class WATER_LEVEL_KELLER36_V2_PARSER : IOT_DEVICE_GENERIC_V2_PARSER
    {
        ...
        public override List<DeviceDefinitionConfigurationDTO> GetDeviceTypeConfigurationItems()
        {
            ...
            configs.Add(new DeviceDefinitionConfigurationDTO() { name = "ZeroHeightCorrection", description = "Correction pressure height(in mm) to get zero level ", symbol = "lv=", value = "0", type = DeviceDefinitionConfigurationDTO.ConfigurationTypeEnum.type_double });
            return configs;
        }
        ...
        public override void AddPostEvent(DeviceMessageDTO message, DeviceDefinitionDTO deviceInfo)
        {
            ...
            string eventName = IOT.METADATA.WATER_LEVEL_KELLER36_V2.MEASUREMENT.DELTA_PRESSURE.AVG.GetType().Name + ".AVG";
            ...
            foreach (DeviceMessageEventDTO telemetry in telemetries)
            {
                ...
                SetMeasurement(message, (DateTime)telemetry.received, telemetry.EDUID, IOT.METADATA.WATER_LEVEL_KELLER36_V2.MEASUREMENT.LEVEL.AVG, level, deviceInfo);
            }
