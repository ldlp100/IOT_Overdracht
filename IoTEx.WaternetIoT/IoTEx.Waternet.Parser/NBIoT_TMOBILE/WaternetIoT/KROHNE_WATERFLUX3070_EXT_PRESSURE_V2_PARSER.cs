...
{
    public class KROHNE_WATERFLUX3070_EXT_PRESSURE_V2_PARSER : IOT_DEVICE_GENERIC_V2_PARSER
    {
        ...
        public override void AddPostEvent(DeviceMessageDTO message, DeviceDefinitionDTO deviceInfo)
        {
            ...
            string eventName = IOT.METADATA.KROHNE_WATERFLUX3070_EXT_PRESSURE_V2.MEASUREMENT.PRESSURE.AVG.GetType().Name + ".AVG";
            ...
            {
                telemetry.value = (telemetry.value/1000);

                //double correctedValue = telemetry.value + ((double)sensorNAPLevel) / 10;

                double correctedValue = telemetry.value +  ((double)sensorNAPLevel / 10000);

                SetMeasurement(message, (DateTime)telemetry.received, telemetry.EDUID,
                    IOT.METADATA.KROHNE_WATERFLUX3070_EXT_PRESSURE_V2.MEASUREMENT.NAP_CORRECTED_PRESSURE.AVG,
