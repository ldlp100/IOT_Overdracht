...
@Component({
  selector: 'app-grid-device-type',
  templateUrl: './grid-device-type.component.html',
  styleUrls: ['./grid-device-type.component.css']
})
export class GridDeviceTypeComponent extends BaseComponent implements OnInit {
 
  public gridView: GridDataResult;
  public editDataItem: DTO.DeviceTypeDTO;
  ...
  public refreshGrid($event) {
    this.service.deviceTypeService.getsGrid(this.gridState).subscribe((result) => {
  ...
  public addHandler($event) {
    this.isNew = true;
    this.editDataItem = new DTO.DeviceTypeDTO();
    //Set Defayult Value
  ...
  public saveHandler(objToSave: any) {
    this.service.deviceTypeService.save(objToSave, this.isNew).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.DeviceTypeDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.deviceTypeService.delete(this.itemToRemove).subscribe((data) => {
        if (data.isOk) {
