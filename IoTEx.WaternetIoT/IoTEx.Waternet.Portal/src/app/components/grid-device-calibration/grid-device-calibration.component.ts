...
@Component({
  selector: 'app-grid-device-calibration',
  templateUrl: './grid-device-calibration.component.html',
  styleUrls: ['./grid-device-calibration.component.css']
})
export class GridDeviceCalibrationComponent extends BaseComponent implements OnInit {
  ...
  public editDataItem: DTO.DeviceCalibrationDTO;
  ...
  public refreshGrid($event) {
    if(this.deviceType != null && this.deviceBatch != null && this.device != null){
      this.service.deviceCalibrationService.getsGrid(this.gridState, this.deviceType.id, this.deviceBatch.id, this.device.id).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.DeviceCalibrationDTO();
  ...
  public saveHandler(objToSave: any) {
    this.service.deviceCalibrationService.save(objToSave, this.isNew, this.deviceType.id, this.deviceBatch.id, this.device.id).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.DeviceCalibrationDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.deviceCalibrationService.delete(this.itemToRemove, this.deviceType.id, this.deviceBatch.id, this.device.id).subscribe((data) => {
        if (data.isOk) {
