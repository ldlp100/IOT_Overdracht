...
@Component({
  selector: 'app-grid-device-configuration',
  templateUrl: './grid-device-configuration.component.html',
  styleUrls: ['./grid-device-configuration.component.css']
})
export class GridDeviceConfigurationComponent extends BaseComponent implements OnInit {
  ...
  public refreshGrid($event) {
    if(this.deviceType != null && this.deviceBatch != null && this.device != null){
      this.service.deviceConfigurationService.getsGrid(this.gridState, this.deviceType.id, this.deviceBatch.id, this.device.id).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.DeviceConfigurationDTO();
  ...
  public saveHandler(objToSave: any) {
    this.service.deviceConfigurationService.save(objToSave, this.isNew, this.deviceType.id, this.deviceBatch.id, this.device.id).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.DeviceConfigurationDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.deviceConfigurationService.delete(this.itemToRemove, this.deviceType.id, this.deviceBatch.id, this.device.id).subscribe((data) => {
        if (data.isOk) {
