...
@Component({
  selector: 'app-grid-supplier',
  templateUrl: './grid-supplier.component.html',
  styleUrls: ['./grid-supplier.component.css']
})
export class GridSupplierComponent extends BaseComponent implements OnInit {
 
  public gridView: GridDataResult;
  public editDataItem: DTO.SupplierDTO;
  ...
  public refreshGrid($event) {
    this.service.supplierService.getsGrid(this.gridState).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.SupplierDTO();
    this.isNew = true;
  }
  ...
  public saveHandler(objToSave: any) {
    this.service.supplierService.save(objToSave, this.isNew).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.SupplierDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.supplierService.delete(this.itemToRemove).subscribe((data) => {
        if (data.isOk) {
