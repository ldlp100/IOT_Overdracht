...
@Component({
  selector: 'app-grid-device-type-firmware-alert',
  templateUrl: './grid-device-type-firmware-alert.component.html',
  styleUrls: ['./grid-device-type-firmware-alert.component.css']
})
export class GridDeviceTypeFirmwareAlertComponent extends BaseComponent implements OnInit {
 
  @Input() public deviceType: DTO.DeviceTypeDTO
  @Input() public deviceTypeFirmware: DTO.DeviceTypeFirmwareDTO

  public gridView: GridDataResult;
  public editDataItem: DTO.DeviceTypeFirmwareAlertDTO;
  ...
  public refreshGrid($event) {
    this.service.deviceTypeFirmwareAlertService.getsGrid(this.gridState, this.deviceType.id, this.deviceTypeFirmware.id).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.DeviceTypeFirmwareAlertDTO();
  ...
  public saveHandler(objToSave: any) {
    this.service.deviceTypeFirmwareAlertService.save(objToSave, this.isNew, this.deviceType.id, this.deviceTypeFirmware.id).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.DeviceTypeFirmwareAlertDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.deviceTypeFirmwareAlertService.delete(this.itemToRemove, this.deviceType.id, this.deviceTypeFirmware.id).subscribe((data) => {
        if (data.isOk) {
