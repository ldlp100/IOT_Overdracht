...
@Component({
  selector: 'app-grid-device-type-firmware',
  templateUrl: './grid-device-type-firmware.component.html',
  styleUrls: ['./grid-device-type-firmware.component.css']
})
export class GridDeviceTypeFirmwareComponent extends BaseComponent implements OnInit {
 
  @Input() public deviceType: DTO.DeviceTypeDTO

  public gridView: GridDataResult;
  public editDataItem: DTO.DeviceTypeFirmwareDTO;
  ...
  public refreshGrid($event) {
    this.service.deviceTypeFirmwareService.getsGrid(this.gridState, this.deviceType.id).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.DeviceTypeFirmwareDTO();
  ...
  public saveHandler(objToSave: any) {
    this.service.deviceTypeFirmwareService.save(objToSave, this.isNew, this.deviceType.id).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.DeviceTypeFirmwareDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.deviceTypeFirmwareService.delete(this.itemToRemove, this.deviceType.id).subscribe((data) => {
        if (data.isOk) {
  ...
  public startGetMetadata(dataItem) {
    this.service.deviceTypeFirmwareService.getMetadata(dataItem, dataItem.deviceTypeId).subscribe((result) => {
