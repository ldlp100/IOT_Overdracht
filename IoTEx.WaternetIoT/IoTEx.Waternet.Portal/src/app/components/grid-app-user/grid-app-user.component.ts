...
@Component({
  selector: 'app-grid-app-user',
  templateUrl: './grid-app-user.component.html',
  styleUrls: ['./grid-app-user.component.css']
})
export class GridAppUserComponent extends BaseComponent implements OnInit {
 
  public gridView: GridDataResult;
  public editDataItem: DTO.AppUserDTO;
  ...
  public refreshGrid($event) {
    this.service.appUserService.getsGrid(this.gridState).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.AppUserDTO();
    this.isNew = true;
  }
  ...
  public saveHandler(objToSave: any) {
    this.service.appUserService.save(objToSave, this.isNew).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.AppUserDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.appUserService.delete(this.itemToRemove).subscribe((data) => {
        if (data.isOk) {
