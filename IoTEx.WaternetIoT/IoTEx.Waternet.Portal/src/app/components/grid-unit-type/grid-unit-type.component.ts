...
@Component({
  selector: 'app-grid-unit-type',
  templateUrl: './grid-unit-type.component.html',
  styleUrls: ['./grid-unit-type.component.css']
})
export class GridUnitTypeComponent extends BaseComponent implements OnInit {
 
  public gridView: GridDataResult;
  public editDataItem: DTO.UnitTypeDTO;
  ...
  public refreshGrid($event) {
    this.service.unitTypeService.getsGrid(this.gridState).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.UnitTypeDTO();
    this.isNew = true;
  }
  ...
  public saveHandler(objToSave: any) {
    this.service.unitTypeService.save(objToSave, this.isNew).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.UnitTypeDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.unitTypeService.delete(this.itemToRemove).subscribe((data) => {
        if (data.isOk) {
