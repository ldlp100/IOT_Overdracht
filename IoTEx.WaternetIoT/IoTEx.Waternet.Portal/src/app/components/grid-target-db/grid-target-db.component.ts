...
@Component({
  selector: 'app-grid-target-db',
  templateUrl: './grid-target-db.component.html',
  styleUrls: ['./grid-target-db.component.css']
})
export class GridTargetDBComponent extends BaseComponent implements OnInit {
 
  public gridView: GridDataResult;
  public editDataItem: DTO.TargetDBDTO;
  ...
  public refreshGrid($event) {
    this.service.targetDBService.getsGrid(this.gridState).subscribe((result) => {
  ...
  public addHandler($event) {
    this.editDataItem = new DTO.TargetDBDTO();
    this.isNew = true;
  }
  ...
  public saveHandler(objToSave: any) {
    this.service.targetDBService.save(objToSave, this.isNew).subscribe((data) => {
  ...
  private updateGridData(updatedObj: DTO.TargetDBDTO, remove: boolean) {
  ...
  public confirmRemove(shouldRemove: boolean): void {
    ...
    if (shouldRemove) {
      this.service.targetDBService.delete(this.itemToRemove).subscribe((data) => {
        if (data.isOk) {
